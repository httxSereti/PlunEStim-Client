    ...
    async def linkLock(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{CHASTER_API_URL}/locks?status=active", headers=self.headers
            ) as response:
                ...
                if not self.lockId:
                    ...
                    Logger.info(
                        f"[Chaster] Lock '{lock['title']}' is now Linked (id='{lock['_id']}')"
                    )
                ...
                for extension in lock["extensions"]:
                    ...
                    if extension["slug"] == "pillory":
                        ...
                        if self.pilloryExtensionId != pilloryExtensionId:
                            ...
                            Logger.info(
                                f"[Chaster] Linked Pillory Extension (id='{pilloryExtensionId}')"
                            )

                    if extension["slug"] == "tasks":
                        self.currentTaskVoteId = extension["userData"][
                            "currentTaskVote"
                        ]
                        Logger.info(
                            f"[Chaster] Linked Tasks Extension (id='{lock['_id']}')"
                        )

                    if extension["slug"] == "wheel-of-fortune":
                        # self.currentTaskVoteId = extension['userData']['currentTaskVote']
                        Logger.info(
                            f"[Chaster] Linked WOF Extension (id='{lock['_id']}')"
                        )
                    ...
                    for vote in data["votes"]:
                        ...
                        if pilloryId not in self.pillories:
                            ...
                            self.pillories[pilloryId] = {
                                ...
                                "nbVotes": 0
                                if PILLORY_PLAY_PREVIOUS_EVENTS
                                else vote[
                                    "nbVotes"
                                ],  # If true, will play all votes previously recorded too
                                "reason": vote["reason"],
            ...
            async with session.post(
                ...
                for event in history["results"]:
                    ...
                    if eventId not in self.eventsHistory:
                        ...
                        if (
                            event["type"] == "wheel_of_fortune_turned"
                            and event["payload"]["segment"]["type"] == "text"
                        ):
                            textPayload = event["payload"]["segment"]["text"]

                            matcher = re.search(
                                "^(\\d|[A-Z][A-Z,a-z][A-Z,a-z]):", textPayload
                            )

                            if matcher:
                                payload = matcher.group(1)
                                Logger.info(
                                    f"[Chaster] Detected dynamic WOF Action (payload='{matcher.group(1)}')"
                                )
                                ...
                                Logger.info(
                                    f"[Chaster] Unrecognized Dynamic WOF Action (payload='{textPayload}')"
                                )

                        # Time Changes (keyholder, user or extension trigger that event)
                        if event["type"] == "time_changed":
                            # not time hidden
                            if "duration" in event["payload"]:
                                voteType: TriggerableEvent = (
                                    TriggerableEvent.CHASTER_TIME_ADD
                                    if event["payload"]["duration"] > 0
                                    else TriggerableEvent.CHASTER_TIME_SUB
                                )

                                Logger.info(
                                    f"[Chaster] {event['role']} changed time (duration='{event['payload']['duration']}', type='{voteType}')"
                                )
                                ...
                                await self.bot.trigger_event(
                                    event_type=voteType, eventData=eventData
                                )
                        ...
                        if event["type"] == "link_time_changed":
                            voteType: TriggerableEvent = (
                                TriggerableEvent.CHASTER_VOTE_ADD
                                if event["payload"]["duration"] > 0
                                else TriggerableEvent.CHASTER_VOTE_SUB
                            )
                            author: str = (
                                event["user"]["username"]
                                if "user" in event
                            ...
                            Logger.info(
                                f"[Chaster] {author} updated time (duration='{event['payload']['duration']}', type='{voteType}')"
                            )
