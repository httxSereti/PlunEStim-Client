...
import { ChartContainer, ChartTooltip, ChartTooltipContent } from "@pes/ui/components/chart";
...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@pes/ui/components/tabs";
...
export default function RealTimeChart() {
    ...
    const formatTime = (time: any) => new Date(time).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" });
                            ...
                            <TabsContent value="line">
                                <ChartContainer config={{ price: { label: "Price", color: "hsl(var(--chart-1))" } }}>
                                    <LineChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                                        ...
                                        <Line type="monotone" dataKey="price" stroke="purple" strokeWidth={2} />
                                    </LineChart>
                                </ChartContainer>
                            </TabsContent>
                            <TabsContent value="area">
                                <ChartContainer config={{ price: { label: "Price", color: "hsl(var(--chart-1))" } }}>
                                    <AreaChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                                        ...
                                        <YAxis tickFormatter={(value) => `$${value.toFixed(2)}`} />
                                        <ChartTooltip content={<ChartTooltipContent formatter={(value) => `$${Number(value).toFixed(2)}`} />} />
                                        <Area type="monotone" dataKey="price" stroke="purple" fillOpacity={0.2} />
                                    </AreaChart>
                                </ChartContainer>
                            </TabsContent>
                        </Tabs>
